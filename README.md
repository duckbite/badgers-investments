...
- **Recommendations:** Manual run produces BUY/SELL/HOLD (and optional WATCH) with rationale; rules + user-configured **Anthropic (Claude)** API key, with deterministic fallback if AI fails.
...
- **Recommendations:** User-configured Anthropic API key (`ai` module); rules engine + deduplication
...
Copy the root env template and set **DynamoDB** table name, region, API port, and (when needed) AI-related env (per-user Anthropic keys via Settings—see `.env.example`).
