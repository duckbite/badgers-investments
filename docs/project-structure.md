...
| `services/api/src/modules/analysis/` | Explore/Library analysis runs vertical slice (DB-148): `analysis-types.ts`, `analysis-run-repository.ts` (Dynamo run/report records), `build-analysis-report-storage-key.ts` (S3 key `{type}-{isoUtc}-{subject}.md`), `build-analysis-report-summary-sentence.ts` (Library card one-line summary; Yahoo `longName`/`shortName` for `technical-analysis`), `fetch-yahoo-instrument-display-name.ts`, `analysis-run-service.ts` (run lifecycle + markdown generation + optional S3 `PutObject`), `register-analysis-routes.ts` — `POST/GET /analysis/runs`, `GET /analysis/reports`, `GET /analysis/reports/:id`, `GET /analysis/reports/:id/export`. |
