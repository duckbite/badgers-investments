...
| `apps/web/src/routes/explore/+page.svelte` | Explore hub UI (DB-144): tools grid, Technical Analysis + Portfolio Builder dialogs, run submission via `/analysis/runs`, and realtime recent-run updates over WebSocket (`analysis` domain events) including live `currentStep` progress labels while processing. |
...
| `apps/web/src/lib/api/analysis.ts` | Typed API client helpers for analysis runs and reports (`/analysis/runs`, `/analysis/reports`); analysis run summaries include realtime `currentStep`; report detail includes optional `storageBundlePrefix`, `storageManifestKey`, `bundleAssetUrls`. |
...
| `apps/web/src/lib/api/recommendations.ts` | Typed client helpers for recommendation endpoints, including per-run realtime `currentStep` feedback while processing. |
...
| `apps/web/src/routes/recommendations/+page.svelte` | Recommendation run history and “Run recommendation” action; realtime list updates via WebSocket events (no interval polling loop), live processing step labels, auto-timeout display, and per-card three-dots actions (`Cancel run` for processing, otherwise `Delete`). |
| `apps/web/src/routes/recommendations/[runId]/+page.svelte` | Single run detail (findings + items) with realtime status updates via WebSocket, reconnect reconciliation, and a live processing step label sourced from backend phases. |
...
| `services/api/src/modules/realtime/realtime-plugin.ts` | Registers `@fastify/websocket` and exposes `/realtime/ws`; pushes both recommendation and analysis run status-change events per authenticated session (connection/degraded + run lifecycle deltas, including `currentStep` changes). |
