...
| `apps/web/src/routes/recommendations/+page.svelte` | Recommendation run history and “Run recommendation” action. |
| `apps/web/src/routes/recommendations/[runId]/+page.svelte` | Single run: rule findings + recommendation items. |
...
| `services/api/src/modules/recommendations/` | Recommendation vertical slice: `build-recommendation-analytics-payload.ts`, `evaluate-recommendation-rules.ts`, `baseline-deterministic-recommendations.ts`, `anthropic-recommendation-llm-adapter.ts` (official `@anthropic-ai/sdk`), `llm-recommendation-client.ts`, `coerce-llm-json-text.ts` (markdown fence + balanced-object extraction before parse), `ai-recommendation-output.ts`, `recommendation-job-queue-repository.ts` (global PK `RECOMMENDATION_JOB_QUEUE` / `REC_JOB#…`), `recommendation-run-service.ts` (async: `PROCESSING` header + queue, worker completes to `COMPLETED`/`FAILED`), `instantiate-recommendation-run-service.ts` (shared wiring for API + scripts), `recommendation-run-repository.ts` (Dynamo `RECOMMENDATION_RUN`, `RULE_FINDING`, `RECOMMENDATION_ITEM`), `register-recommendations-routes.ts` — `POST /recommendations/runs`, `GET /recommendations/runs`, `GET /recommendations/runs/:id`, `GET /recommendations/latest-summary` (first **completed** run in recent window). |
...
| Path | Description |
|------|-------------|
| `workers/worker/` | Worker runtime: `src/index.ts` runs `pnpm --filter api recommendation-queue:process` from repo root (process queued recommendation runs). Scripts: `dev`, `build`, `start`, `lint`, `clean`. Build output: `dist/`. |
