...
| `apps/web/src/routes/library/+page.svelte` | Library page (DB-145): search/filter + My/All tabs, report cards, report viewer modal with **sanitized markdown-to-HTML rendering** (`marked` + `DOMPurify`), markdown export via `/analysis/reports/:id/export`. |
| `apps/web/src/lib/api/analysis.ts` | Typed API client helpers for analysis runs and reports (`/analysis/runs`, `/analysis/reports`). |
...
| `apps/web/src/routes/dashboard/+page.svelte` | Wealth overview: latest snapshot KPIs, top holdings, sector allocation, freshness, **latest recommendation summary** (links to run detail), Chart.js charts (hidden when anonymized), links. |
...
| `services/api/src/config/get-analysis-report-storage-config.ts` | Analysis report storage env parser: `API_REPORTS_BUCKET_NAME` + AWS region fallback for S3 persistence. |
...
| `services/api/src/modules/analysis/` | Explore/Library analysis runs vertical slice (DB-148): `analysis-types.ts`, `analysis-run-repository.ts` (Dynamo run/report records), `analysis-run-service.ts` (run lifecycle + markdown generation + optional S3 `PutObject`), `register-analysis-routes.ts` — `POST/GET /analysis/runs`, `GET /analysis/reports`, `GET /analysis/reports/:id`, `GET /analysis/reports/:id/export`. |
...
| Path | Description |
|------|-------------|
| `workers/worker/` | Daily worker runtime (`src/index.ts`, `src/lambda-handler.ts`) for scheduled jobs; recommendation processor runtime is deployed from API bundle (`services/api/src/lambda-recommendation-processor-handler.ts`). |
...
| `infra/terraform/modules/api_lambda/` | API Lambda, HTTP API, regional ACM + custom domain for `api_domain` (Route53 records when `route53_zone_id` set). Module variable `api_node_env` (default `production`) sets `API_NODE_ENV` / `NODE_ENV`; optional `api_ai_model_anthropic` sets `API_AI_MODEL_ANTHROPIC`. Sensitive env vars `COOKIE_SECRET`, `API_AI_SETTINGS_SECRET`, `API_PRIVACY_SECRET` are passed from the caller (same strings as `secrets` module / SM JSON) so plan does not read a stale secret version. |
