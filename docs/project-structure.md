...
| `services/api/src/modules/recommendations/` | Recommendation vertical slice: `build-recommendation-analytics-payload.ts`, `evaluate-recommendation-rules.ts`, `baseline-deterministic-recommendations.ts`, `anthropic-recommendation-llm-adapter.ts` (official `@anthropic-ai/sdk`), `llm-recommendation-client.ts`, `coerce-llm-json-text.ts` (markdown fence + balanced-object extraction before parse), `ai-recommendation-output.ts`, `recommendation-run-service.ts`, `recommendation-run-repository.ts` (Dynamo `RECOMMENDATION_RUN`, `RULE_FINDING`, `RECOMMENDATION_ITEM`), `register-recommendations-routes.ts` — `POST /recommendations/runs`, `GET /recommendations/runs`, `GET /recommendations/runs/:id`, `GET /recommendations/latest-summary`. |
